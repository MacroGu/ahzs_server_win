...
#define MSGSIZE    1024
// 用 select 转换 epoll 
CEpollServer::CEpollServer() :  m_fds(), the_world(NULL), m_unMailboxId(0), m_bShutdown(false), m_unMaxPlutoCount(0)
{
...
int CEpollServer::StartServer(const char* pszAddr, uint16_t unPort)
{
	...
	/* 添加这个 fd 到 select 的 fd_set 中去*/
	FD_ZERO(&allset);		// 将 allset 初始化空集
	FD_SET(fd, &allset);	// 将这个 fd 添加进去
...
int CEpollServer::ConnectMailboxs(const char* pszCfgFile)
{
	...
	for(; iter != mbs.end(); )
	{
		...
		int nRet = pmb->ConnectServer(0);	// 参数 0 是随便写的
		if(nRet != 0)
		{
			return nRet;
		}
...
int CEpollServer::Service(const char* pszAddr, unsigned int unPort)
{
	...
	while (!m_bShutdown)
	{
		...
		int ret = select(maxfd + 1, &rset, NULL, NULL, NULL);//每隔一段时间，检查可读性的套接口
		if (ret < 0)		// 排除掉错误的返回
		{
...
int CEpollServer::HandleNewConnection(int fd)
{
	...
	FD_SET(new_fd, &allset);	/* add new descriptor to set */
	if (new_fd > maxfd)
		maxfd = new_fd; /* for select */
	return 0;
}
