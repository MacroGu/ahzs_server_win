...
#define NS_MOGO_BEGIN                     namespace mogo {
#define NS_MOGO_END                       }
#define USING_NS_MOGO                     using namespace mogo
...
public:
	...
	~MyLock(){ if (!m_Mutex) return; pthread_mutex_destroy(&m_Mutex); };
	void Lock(){ pthread_mutex_lock(&m_Mutex); };
	void Unlock(){ if (!m_Mutex) return; pthread_mutex_unlock(&m_Mutex); };
};
NS_MOGO_END
#endif
